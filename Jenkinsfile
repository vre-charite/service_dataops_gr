pipeline {
    agent { label 'small' }
    environment {
      imagename_dev = "10.3.7.221:5000/dataops-gr"
      imagename_staging = "10.3.7.241:5000/dataops-gr"
    ...
    stage('Git clone for dev') {
        ...
        steps{
          script {
          git branch: "k8s-dev",
              url: 'https://git.indocresearch.org/platform/service_dataops_gr.git',
              credentialsId: 'lzhao'
            }
        }
    }
      ...
      steps{
        script {
            docker.withRegistry('http://10.3.7.221:5000', registryCredential) {
                customImage = docker.build("10.3.7.221:5000/dataops-gr:${env.BUILD_ID}")
                customImage.push()
            }
        }
      }
      ...
      steps{
        script {
            docker.withRegistry('http://10.3.7.241:5000', registryCredential) {
                customImage = docker.build("10.3.7.241:5000/dataops-gr:${env.BUILD_ID}")
                customImage.push()
            }
        }
      }
